...
class ParserModel(nn.Module):
    ...
    def __init__(self, embeddings, n_features=36,
        ...
        # Trainable weight matrix mapping flattened embeddings to the hidden layer
        self.embed_to_hidden_weight = nn.Parameter(
            torch.empty(self.embed_size * self.n_features, self.hidden_size)
        )

        # Xavier uniform initialization for embed-to-hidden weights to stabilize training
        nn.init.xavier_uniform_(self.embed_to_hidden_weight)

        # Trainable bias vector for the hidden layer
        self.embed_to_hidden_bias = nn.Parameter(
            torch.empty(self.hidden_size)
        )

        # Uniform initialization for hidden layer bias values
        nn.init.uniform_(self.embed_to_hidden_bias)

        # Dropout layer to randomly zero hidden units during training for regularization
        self.dropout = nn.Dropout(p=self.dropout_prob)

        # Trainable weight matrix mapping hidden layer activations to output class logits
        self.hidden_to_logits_weight = nn.Parameter(
            torch.empty(self.hidden_size, self.n_classes)
        )

        # Xavier uniform initialization for hidden-to-logits weights to stabilize training
        nn.init.xavier_uniform_(self.hidden_to_logits_weight)

        # Trainable bias vector for the output (logits) layer
        self.hidden_to_logits_bias = nn.Parameter(
            torch.empty(self.n_classes)
        )

        # Uniform initialization for output layer bias values
    ...
    def forward(self, w):
        ...
        # Look up and flatten embeddings for each word index in the input
        x = self.embedding_lookup(w)

        # Apply first linear transformation from embeddings to hidden layer
        h = torch.matmul(x, self.embed_to_hidden_weight) + self.embed_to_hidden_bias

        # Apply ReLU nonlinearity to introduce non-linear features
        h = F.relu(h)

        # Apply dropout to hidden activations for regularization during training
        h = self.dropout(h)

        # Apply second linear transformation to produce class logits
