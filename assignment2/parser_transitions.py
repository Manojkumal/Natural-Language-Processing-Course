...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        # LEFT-ARC: add a dependency from top of stack to second-top, then remove second-top from stack
        elif transition == "LA":
            self.dependencies.append((self.stack[-1], self.stack.pop(-2)))

        # RIGHT-ARC: add a dependency from second-top of stack to top, then remove top of stack
        else:
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        for parse_obj, transition in zip(batch_parses, transitions):
            parse_obj.parse_step(transition)
            if len(parse_obj.buffer) == 0 and len(parse_obj.stack) == 1:
